...
-- 8. V1.0 MVP: 标签、职业匹配、行为和推荐日志
ALTER TABLE websites ADD COLUMN IF NOT EXISTS quality_score DECIMAL(4,2) DEFAULT 0.80 AFTER clicks;
ALTER TABLE websites ADD COLUMN IF NOT EXISTS popularity_score DECIMAL(4,2) DEFAULT 0.00 AFTER quality_score;
ALTER TABLE websites ADD COLUMN IF NOT EXISTS freshness_score DECIMAL(4,2) DEFAULT 0.60 AFTER popularity_score;

CREATE TABLE IF NOT EXISTS tags (
    id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(80) NOT NULL,
    slug VARCHAR(80) NOT NULL UNIQUE,
    type VARCHAR(30) DEFAULT 'interest',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE IF NOT EXISTS site_occupations (
    ...
    weight DECIMAL(4,2) DEFAULT 1.00,
    FOREIGN KEY (site_id) REFERENCES websites(id) ON DELETE CASCADE,
    INDEX idx_site_occupation (site_id, occupation)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS user_behavior (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_user_behavior_user (user_id, created_at),
    INDEX idx_user_behavior_site (website_id, created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS recommendation_logs (
    ...
    source VARCHAR(30) DEFAULT 'mvp',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_recommendation_user (user_id, created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT IGNORE INTO tags (name, slug, type) VALUES
('AI工具', 'ai-data', 'interest'),
('学习', 'self-learning', 'interest'),
('开发', 'backend', 'occupation'),
('设计', 'ui-design', 'occupation'),
('办公效率', 'efficiency', 'interest'),
('求职', 'job-hunting', 'interest'),
('开源', 'open-source', 'interest');

-- 9. V1.0 MVP: 网站详情页评分与评论
CREATE TABLE IF NOT EXISTS site_comments (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_site_comments_site (website_id, created_at),
...
CREATE TABLE IF NOT EXISTS site_ratings (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uniq_site_rating_user (website_id, user_id),
    INDEX idx_site_ratings_site (website_id),
    FOREIGN KEY (website_id) REFERENCES websites(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 10. V1.0 MVP: 举报与 DMCA 后台治理
CREATE TABLE IF NOT EXISTS reports (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NULL,
    target_type VARCHAR(40) NOT NULL DEFAULT 'website',
