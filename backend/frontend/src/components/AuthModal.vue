    ...
    <div class="auth-modal" @click.stop>
      ...
      <transition name="fade-slide" mode="out-in">
        ...
        <div v-if="stage === 'methods'" key="methods" class="vertical-layout">
          ...
          <button class="method-btn register" @click="switchTo('register')"
            style="margin-top: 10px; border-color: #3b82f6; color: #3b82f6;">
            ✨ 新用户注册
          </button>
          <button class="link-btn" @click="switchTo('reset')" style="margin-top: 6px; font-size: 12px; color: #94a3b8;">
            忘记密码？
          </button>
        </div>

        <!-- 手机号登录 -->
        <div v-else-if="stage === 'mobile'" key="mobile" class="vertical-layout">
        ...
        <div v-else-if="stage === 'email'" key="email" class="vertical-layout">
          ...
          <input type="email" v-model="loginForm.email" placeholder="请输入邮箱地址" class="auth-input" />
          <input type="password" v-model="loginForm.password" placeholder="请输入密码" class="auth-input" />
          <button class="btn-submit" @click="handleEmailLogin">确认登录</button>
          <button class="link-btn" @click="switchTo('methods')">返回其他方式</button>
        </div>

        <!-- 新用户注册 -->
        <div v-else-if="stage === 'register'" key="register" class="vertical-layout">
          ...
          <div class="verify-code-row">
            ...
            <button class="get-code-btn" :disabled="regCountdown > 0" @click="sendRegCode">
        ...
        <div v-else-if="stage === 'reset'" key="reset" class="vertical-layout">
          ...
          <p style="font-size: 13px; color: #64748b; text-align: center; margin-bottom: 12px;">
            输入注册邮箱，我们将发送重置链接
          </p>

          <!-- 步骤 1：输入邮箱 -->
          <template v-if="resetStep === 1">
            <input type="email" v-model="resetForm.email" placeholder="请输入注册时使用的邮箱" class="auth-input" />
            <button class="btn-submit" @click="sendResetCode">发送重置验证码</button>
          </template>

          <!-- 步骤 2：输入验证码 + 新密码 -->
          <template v-else-if="resetStep === 2">
            <div class="verify-code-row">
              <input type="text" v-model="resetForm.code" placeholder="6位验证码" class="auth-input small" />
              <button class="get-code-btn" :disabled="resetCountdown > 0" @click="sendResetCode">
                {{ resetCountdown > 0 ? `${resetCountdown}s 后重发` : '重新发送' }}
              </button>
            </div>
            <input type="password" v-model="resetForm.newPassword" placeholder="请输入新密码（至少6位）" class="auth-input" />
            <button class="btn-submit" @click="confirmResetPassword">重置密码并登录</button>
          </template>
          <button class="link-btn" @click="switchTo('methods')">返回其他方式</button>
        </div>
      </transition>
...
const regForm = reactive({
  username: '', email: '', password: '', code: '', agreeTerms: false
})
...
const sendRegCode = async () => {
  if (!regForm.email.includes('@')) return emit('toast', '⚠️ 请输入有效的邮箱格式！', 'error')
  ...
  try {
    ...
    if (res.data.code === 0) {
      ...
      clearInterval(regTimer); regCountdown.value = 0
...
const handleRegister = async () => {
  if (!regForm.agreeTerms) return emit('toast', '⚠️ 请先阅读并勾选用户协议和隐私政策！', 'error')
  if (!regForm.username || !regForm.email || !regForm.password || !regForm.code) {
  ...
  try {
    ...
    if (res.data.code === 0) {
      ...
      regForm.username = ''; regForm.email = ''; regForm.password = ''; regForm.code = ''; regForm.agreeTerms = false
      switchTo('email')
    } else {
      emit('toast', '❌ 注册失败：' + res.data.msg, 'error')
    }
  } catch (error) {
    emit('toast', '❌ 网络或服务错误：' + (error.response?.data?.msg || '请检查控制台'), 'error')
  }
}
...
const handleEmailLogin = async () => {
  ...
  try {
    ...
    emit('toast', '❌ ' + (error.response?.data?.msg || '账号或密码错误'), 'error')
  }
}
...
const sendResetCode = async () => {
  if (!resetForm.email.includes('@')) return emit('toast', '⚠️ 请输入有效的邮箱格式！', 'error')
  if (resetCountdown.value > 0) return
  resetCountdown.value = 60
  resetTimer = setInterval(() => { resetCountdown.value--; if (resetCountdown.value <= 0) clearInterval(resetTimer) }, 1000)
  try {
    ...
    if (res.data.code === 0) {
      ...
      clearInterval(resetTimer); resetCountdown.value = 0
      emit('toast', '❌ ' + res.data.msg, 'error')
    }
  } catch (error) {
    clearInterval(resetTimer); resetCountdown.value = 0
...
const confirmResetPassword = async () => {
  ...
  try {
    const res = await authAPI.resetPassword(resetForm.email, resetForm.code, resetForm.newPassword)
    if (res.data.code === 0) {
      emit('toast', '🎉 密码重置成功！请使用新密码登录', 'success')
      // 清空表单
      resetForm.email = ''; resetForm.code = ''; resetForm.newPassword = ''
      resetStep.value = 1
      switchTo('email')
    } else {
      emit('toast', '❌ ' + res.data.msg, 'error')
    ...
    emit('toast', '❌ ' + (error.response?.data?.msg || '重置失败，请检查验证码'), 'error')
  }
}
</script>
