...
const checks = [
  ...
  [
    "Header creates one store reference",
    /const aiAssistantStore = useAiAssistantStore\(\)/.test(header),
  ],
  [
    "Header AI entry stays available for both authentication states",
    header.includes('data-testid="ai-assistant-entry"') &&
      /class="nav-item nav-ai-login-entry"[\s\S]*?@click="handleAiAssistantEntry"/.test(
        header,
      ) &&
  ...
  [
    "Header does not mount an AI panel or call an AI API",
    !header.includes("<AiSiteAssistant") && !header.includes("site-recommend"),
  ],
  [
    "Header does not use an AI anchor",
    !header.includes("/#ai") && !header.includes("scrollToSection('ai')"),
  ],
  [
    "Home imports the shared AI assistant store",
    home.includes("useAiAssistantStore"),
  ],
  [
    "Home creates one shared store reference",
    /const aiAssistantStore = useAiAssistantStore\(\)/.test(home),
  ],
  [
  ...
  [
    "Home keeps the signed-out 知航AI prompt and login label",
    home.includes("登录并使用知航AI助手"),
  ],
  [
    "Home adds the signed-in 知航AI prompt and entry label",
    home.includes("询问知航AI助手"),
  ],
  [
    "Home prompt uses the shared AI entry handler",
    /class="ai-login-prompt[^"]*"[\s\S]*?@click="handleAiAssistantEntry"/.test(
      home,
    ),
  ],
  [
  ...
  [
    ...
    /watch\(careerAuthIdentity,\s*(?:async\s*)?\(identity[^)]*\)\s*=>\s*\{\s*if \(!identity\) \{[\s\S]*?clearFavoriteState\(\);\s*aiAssistantStore\.closeAssistant\(\)/s.test(
      home,
    ),
  ],
  [
  ...
  [
    "No direct auth token reads or AI requests were added to entry points",
    !/localStorage\.(?:access_token|token)|Authorization|site-recommend|\/api\/ai/.test(
      `${header}\n${home}`,
    ),
  ],
  [
    "The removed hot section remains absent",
    !home.includes('id="hot"') && !header.includes("/#hot"),
  ],
  [
    "Career SiteCards keep recommendation reasons enabled",
    home.includes(':show-reason="true"'),
  ],
];
